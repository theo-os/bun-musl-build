...
vendor-without-check: npm-install node-fallbacks runtime_js fallback_decoder bun_error mimalloc picohttp zlib boringssl libarchive libbacktrace lolhtml usockets uws base64
...
.PHONY: devcontainer
devcontainer: clone-submodules mimalloc zlib libarchive boringssl picohttp identifier-cache node-fallbacks jsc-bindings-headers api analytics bun_error fallback_decoder jsc-bindings-mac dev runtime_js_dev libarchive libbacktrace lolhtml usockets uws base64
...
jsc-bindings-headers:
	...
	$(ZIG) build -Dtarget=native-linux-musl headers-obj
...
jsc-build-linux-compile-config:
	...
	cd $(WEBKIT_RELEASE_DIR) && \
		cmake \
			...
			-DENABLE_STATIC_JSC=ON \
			-DENABLE_WEBGPU=ON \
			-DENABLE_NAVIGATOR_STANDALONE=ON \
			-DCMAKE_BUILD_TYPE=relwithdebuginfo \
			-DUSE_THIN_ARCHIVES=OFF \
			-DENABLE_FTL_JIT=ON \
			-DENABLE_REMOTE_INSPECTOR=ON \
